...
const GoogleSimulation: React.FC<GoogleSimulationProps> = ({ searchType = 'terrell' }) => {
  ...
  useEffect(() => {
    trackPageView('terrell', currentPage, activeTab);
  }, []);

  // Track session end on beforeunload
  useEffect(() => {
    const handleBeforeUnload = () => {
      trackSessionEnd('terrell', currentPage, activeTab);
    };
                  ...
                  return (
                    <React.Fragment key={result.id}>
                      <ResultCard
                        result={result}
                        onOpen={(result) => {
                          trackResultClick(result.id, result.platform, result.displayName, 'terrell');
