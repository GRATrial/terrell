...
const GoogleSimulation: React.FC<GoogleSimulationProps> = ({ searchType = 'terrell' }) => {
  const [searchQuery, setSearchQuery] = useState('Terrell Jackson');
  ...
  useEffect(() => {
    trackPageView('terrell', currentPage, activeTab, undefined, prolificParams);
  }, []);

  // Track session end on beforeunload
  useEffect(() => {
    const handleBeforeUnload = () => {
      trackSessionEnd('terrell', currentPage, activeTab, undefined, prolificParams);
    };
  ...
  // Get results for Terrell Jackson
  const allResults = useMemo(() => {
    console.log('Loading RESULTS_Terrell_Jackson:', RESULTS_Terrell_Jackson);
    if (!RESULTS_Terrell_Jackson || RESULTS_Terrell_Jackson.length === 0) {
      console.error('RESULTS_Terrell_Jackson is empty or undefined!');
      return [];
    }
    console.log('Loaded', RESULTS_Terrell_Jackson.length, 'results');
    return RESULTS_Terrell_Jackson;
  }, []);
  ...
  if (!allResults || allResults.length === 0) {
    return (
      <div style={{ padding: '20px', textAlign: 'center' }}>
        ...
        <p>Terrell Jackson results are not available.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#fff' }}>
      <TopBar searchQuery={searchQuery} onSearchChange={(q) => { setSearchQuery(q); trackSearch(q, 'terrell'); }} isDark={isDark} />
      <Tabs activeTab={activeTab} onTabChange={(tab) => { setActiveTab(tab); trackTabChange(tab, 'terrell'); }} isDark={isDark} />
        ...
        <div style={{ paddingTop: '20px', paddingBottom: '8px' }}>
          <button
            onClick={() => {
              ...
              trackEventBeacon({ eventType: 'click', elementType: 'done_searching_button', elementText: 'Done Searching', persona: 'terrell', page: currentPage, tab: activeTab });
        ...
        <div style={{ display: 'flex', gap: '32px' }}>
          ...
          <div style={{ flex: '1', minWidth: 0 }}>
            ...
            <div style={{ color: '#70757a', fontSize: '14px', marginBottom: '16px' }}>
              About 2,450,000 results (0.33 seconds)
            </div>
                  ...
                  return (
                    <React.Fragment key={result.id}>
                      <ResultCard
                        result={result}
                        onOpen={(result) => {
                          trackResultClick(result.id, result.platform, result.displayName, 'terrell', undefined, prolificParams);
            ...
            {filteredResults.length > 0 && totalPages > 1 && (
              <div style={{ 
                ...
                {currentPage > 1 && (
                  <button
                    onClick={() => { const p = currentPage - 1; setCurrentPage(p); trackPagination(p, 'terrell'); }}
                    style={{
                      padding: '8px 16px',
                ...
                {getPageNumbers().map((page, index) => {
                  ...
                  return (
                    <button
                      key={pageNum}
                      onClick={() => { setCurrentPage(pageNum); trackPagination(pageNum, 'terrell'); }}
                      style={{
                        minWidth: '40px',
                        height: '40px',
                        padding: '0 8px',
            ...
            {activeTab === 'All' && filteredResults.length > 0 && (
              <PeopleAlsoSearchFor 
                searches={getRelatedSearches(searchQuery || 'Terrell Jackson')} 
